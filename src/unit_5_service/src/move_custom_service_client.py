...
import rospkg
import rospy
from unit_5_service.srv import mycostumservicemessage, mycostumservicemessageRequest


rospy.init_node('service_move_custom_client') # Initialise a ROS node with the name service_client
rospy.wait_for_service('/move_bb8_in_circle_custom') # Wait for the service client /move_bb8_in_circle_custom to be running
move_bb8_in_circle_service_client = rospy.ServiceProxy('/move_bb8_in_circle_custom', mycostumservicemessage) # Create the connection to the service
move_bb8_in_circle_request_object = mycostumservicemessageRequest() # Create an object of type EmptyRequest
...
"""
duration=int(input("type move duration time: "))
move_bb8_in_circle_request_object.duration = duration
